...
final class AdminTypographyController extends Controller
{
    ...
    private function hydrateTypography(array $input): array
    {
        return [
            ...
            'body_font_weight' => $this->sanitizeFontWeight($input['body_font_weight'] ?? 400, 400),
            ...
            'heading_font_weight' => $this->sanitizeFontWeight($input['heading_font_weight'] ?? 600, 600),
            ...
            'menu_font_weight' => $this->sanitizeFontWeight($input['menu_font_weight'] ?? 500, 500),
            ...
            'submenu_font_weight' => $this->sanitizeFontWeight($input['submenu_font_weight'] ?? 500, 500),
    ...
    private function validateTypography(array $form): array
    {
        ...
        foreach ([
            ...
            if (str_ends_with($field, '_font_weight')) {
                if (!$this->isSafeFontWeight($form[$field])) {
                    $errors[$field] = 'Wybierz poprawną grubość fontu.';
                }
    ...
    private function sanitizeFontWeight(mixed $value, int $default): int
    {
        $weight = (int) $value;

        return $this->isSafeFontWeight($weight) ? $weight : $default;
    }

    private function isSafeFontWeight(mixed $value): bool
    {
        return in_array((int) $value, [100, 200, 300, 400, 500, 600, 700, 800, 900], true);
    }
