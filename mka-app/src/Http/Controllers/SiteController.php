...
final class SiteController extends Controller
{
    ...
    public function notFound(string $message = 'Nie znaleziono strony.'): string
    {
        ...
        return $this->render('site/404', [
            ...
            'localeSwitch' => $this->localeSwitchUrls('pl'),
            'breadcrumbs' => [],
            'seo' => [
    ...
    private function renderAccordion(
        ...
        return $this->render('site/accordion', [
            ...
            'settings' => $settings,
            'navGroups' => $navGroups,
            'localeSwitch' => $this->localeSwitchUrls($locale, $activeContext),
            'seo' => $activeContext
    ...
    private function localeSwitchUrls(string $locale, ?array $activeContext = null): array
    {
        $currentRequestUri = $_SERVER['REQUEST_URI'] ?? '/' . $locale;
        $currentPath = parse_url($currentRequestUri, PHP_URL_PATH) ?: '/' . $locale;
        $currentQuery = parse_url($currentRequestUri, PHP_URL_QUERY);
        $pathWithoutLocale = preg_replace('#^/(pl|en)(?=/|$)#', '', $currentPath) ?: '';

        $urls = [];

        foreach (['pl', 'en'] as $targetLocale) {
            $targetPath = '/' . $targetLocale . $pathWithoutLocale;

            if (
                $activeContext !== null
                && !empty($activeContext['id'])
                && ($translatedEntry = $this->app->contentRepository()->findLocalizedById((int) $activeContext['id'], $targetLocale))
            ) {
                $targetPath = $this->app->contentService()->relativeUrl($translatedEntry, $targetLocale);
            }
