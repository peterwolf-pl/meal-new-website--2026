...
final class NavigationRepository
{
    ...
    public function replaceLocaleWithTree(string $locale, array $items): void
    {
        if (!$this->tableExists()) {
            return;
        }

        $this->database->transaction(function (PDO $pdo) use ($locale, $items): void {
            $delete = $pdo->prepare('DELETE FROM navigation_items WHERE locale = :locale');
            $delete->execute(['locale' => $locale]);
            ...
            $walk = function (array $nodes, ?int $parentId = null) use (&$walk, $insert, $locale, $pdo): void {
                foreach ($nodes as $node) {
                    $insert->execute([
                        ...
                        'slug' => ((string) ($node['slug'] ?? '') ?: null),
                        'href' => ((string) ($node['href'] ?? '') ?: null),
                        'item_kind' => (string) ($node['item_kind'] ?? 'link'),
                        'section_key' => ((string) ($node['section_key'] ?? '') ?: null),
                        'content_type' => ((string) ($node['content_type'] ?? '') ?: null),
