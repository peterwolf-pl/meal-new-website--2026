...
final class OpenAiTranslationService
{
    ...
    public function translateMenuItemsToEnglish(array $items): array
    {
        if (!$this->isConfigured()) {
            throw new RuntimeException('Brakuje klucza OpenAI API.');
        }
        ...
        $response = $this->postJson(self::API_URL, [
            ...
            'input' => [
                [
                    'role' => 'system',
                    'content' => implode("\n", [
                        'You translate Polish website navigation labels into natural English.',
                        'Return only the JSON object required by the schema.',
                        'Preserve empty titles as empty strings.',
                        'For slug, return a concise English URL slug using lowercase latin letters, digits, and hyphens only.',
                        'Keep casing style similar to the source when reasonable.',
                    ]),
                ],
                [
                    'role' => 'user',
        ...
        $decoded = $this->decodeStructuredTranslation($response);
        $translations = [];
        ...
        foreach ($decoded['items'] ?? [] as $item) {
            ...
            $id = (int) $item['id'];
            $source = $items[$id] ?? null;
            if (!is_array($source)) {
                continue;
            }

            $translations[$id] = [
                'label' => $this->preserveMenuCasing(trim((string) ($item['label'] ?? '')), (string) ($source['label'] ?? '')),
                'title' => $this->preserveMenuCasing(trim((string) ($item['title'] ?? '')), (string) ($source['title'] ?? '')),
                'slug' => $this->app->contentService()->normalizeSlug((string) ($item['slug'] ?? '')),
            ];

            if (trim((string) ($source['label'] ?? '')) === '') {
                $translations[$id]['label'] = '';
            }
            ...
            if (trim((string) ($source['slug'] ?? '')) === '') {
                $translations[$id]['slug'] = '';
            }
        }
    ...
    private function preserveMenuCasing(string $translated, string $source): string
    {
        if ($translated === '' || $source === '') {
            return $translated;
        }

        return $source === mb_strtoupper($source)
            ? mb_strtoupper($translated)
            : $translated;
    }
}
