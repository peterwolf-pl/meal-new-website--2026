    ...
    <section class="admin-form-card">
        <div class="admin-form-card-head">
            <div>
                ...
                <p class="helper-note">Jedno logowanie Google może automatycznie wykryć zasoby tej domeny w GA4, Search Console i GTM.</p>
            </div>
            <div class="admin-actions">
                <?php if ($googleConnection['client_configured']): ?>
                    <a class="button-primary" href="/admin/settings/google/connect">
                        <?= $googleConnection['connected'] ? 'Połącz ponownie z Google' : 'Połącz z Google' ?>
                    </a>
        ...
        <?php if (!$googleConnection['client_configured']): ?>
            <p class="form-error">
                Auto-konfiguracja OAuth wymaga ustawienia <code>GOOGLE_OAUTH_CLIENT_ID</code> i <code>GOOGLE_OAUTH_CLIENT_SECRET</code>.
        ...
        <?php elseif ($googleConnection['connected']): ?>
            <p class="helper-note">
                Połączono<?= !empty($googleConnection['email']) ? ': ' . e($googleConnection['email']) : '.' ?>
        ...
        <?php else: ?>
            <p class="helper-note">
                Po zalogowaniu system spróbuje dopasować zasoby po domenie <code><?= e(parse_url((string) ($googleConnection['callback_url'] ?? ''), PHP_URL_HOST) ?: '') ?></code>
                oraz nazwie serwisu. Jeśli czegoś nie znajdzie, poniższe pola nadal możesz uzupełnić ręcznie.
            </p>
        <?php endif; ?>
